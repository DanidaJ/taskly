...
async def log_task_history_entry(entry: dict):
    ...
    try:
        ...
        print(f"Warning: failed to write task history entry: {e}")


def is_task_missed(task_data: dict, plan_date: str) -> bool:
    """Check if a task's scheduled time has passed without completion.
    Returns True if task should be marked as missed."""
    ...
    if not scheduled_end:
        return False
    try:
        end_str = normalize_time_format(scheduled_end)
        if not end_str:
            return False
        end_dt = datetime.strptime(f"{plan_date} {end_str}", '%Y-%m-%d %H:%M')
        grace_period = timedelta(minutes=15)
        return datetime.now() > (end_dt + grace_period)
    except Exception:
        return False
...
async def get_daily_plan(
    ...
    try:
        ...
        for t in db_tasks:
            ...
            if api_status == "pending" and is_task_missed(t, date):
                api_status = "missed"
    ...
    try:
        ...
        if existing_plan:
            ...
            if old_tasks:
                ...
                try:
                    ...
                    print(f"Warning: failed to archive replaced tasks: {e}")

            # Delete old planned tasks
            await db.delete_planned_tasks(plan_id)
            # Update plan data
            plan_data = {
                "id": plan_id,
                "user_id": user_id,
        ...
        if plan.tasks:
            for idx, task in enumerate(plan.tasks):
                ...
                # Print debug info
                print(f"DEBUG: task_id from frontend: '{task.task_id}', task_id_value: {task_id_value}")
                ...
                task_data = {
                    ...
                    "cognitive_load": "routine",  # Default value
        ...
        try:
            ...
            for rt in recurring:
                ...
                rt_task = {
                    ...
                    "cognitive_load": rt.get('cognitive_load', 'routine'),
            ...
            print(f"Warning: Failed to inject recurring tasks: {e}")
        
        # Save all tasks (user tasks + recurring tasks)
        if tasks_to_save:
            saved_tasks_data = await db.save_planned_tasks(tasks_to_save)
...
async def get_daily_plans_range(
    ...
    try:
        ...
        for plan in plans:
            ...
            for t in db_tasks:
                api_status = db_status_to_api(t.get("status", "not_started"))
                if api_status == "pending" and is_task_missed(t, str(plan_date)):
                    api_status = "missed"
                tasks.append(PlannedTask(
                    id=t["id"],
                    task_id=t.get("task_id") or "",  # Return empty string if NULL, not the task id
